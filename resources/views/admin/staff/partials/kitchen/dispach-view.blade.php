...
.order-body {
    flex-grow: 1;
    overflow-y: auto; /* make only inner content scrollable */
    padding-right: 5px; /* for scrollbar space */
}

/* optional: make scrollbar thin and nice */
...
.btn-preparing:hover {
    ...
    /* =====================================================
   📌 TABLET VIEW — Medium Screens (Width ≤ 1024px)
   2 cards per row
===================================================== */
...
/* =====================================================
   📌 SMALL TABLETS / MOBILE (Width ≤ 768px)
   1 card per row
===================================================== */
  ...
  <script>
      function getKdsOrders() {
       ...
       let lastOrderId = $('input[id^="order_id"]').last().val();
          // console.log('Refreshing KDS orders...');
          $.get("{{ route('staff.dispach-refresh') }}", { branch_id: branchId, order_id: lastOrderId }, function(res) {
              $('#kdsContainer').append(res.html);
          });
      }
    ...
    `;
        ...
        `;
              ...
              if ($(this).find('.addon-list li').length > 0) {
                  printContent +=
                      `<div style="margin-left:10px;">Addons:</div><ul style="margin:0; padding-left:15px;">`;
              ...
              if ($(this).find('.removed-ingredient-list li').length > 0) {
                  printContent +=
                      `<div style="margin-left:10px;">Removed:</div><ul style="margin:0; padding-left:15px;">`;
